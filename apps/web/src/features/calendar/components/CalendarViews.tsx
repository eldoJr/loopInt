        ...
        {days.map((day, index) => (
          ...
          >
            ...
            <div className="space-y-1">
              {events
                .filter(event => event.date.toDateString() === day.dateObj.toDateString())
                .slice(0, 3)
                .map(event => (
                  <div
                    key={event.id}
...
const CalendarViews = ({ view, currentDate, events, onDateClick }: CalendarViewsProps) => {
  const renderView = () => {
    switch (view) {
      case 'Day':
      ...
      case 'Month':
      default:
        return <MonthView currentDate={currentDate} events={events} onDateClick={onDateClick} />;
    }
  };
