  ...
  const KPICard = ({
    ...
    return (
      <div className="bg-white dark:bg-gray-900/50 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800/50 p-6 relative backdrop-blur-sm hover:shadow-md transition-all duration-200">
        <div
          className={`absolute top-0 left-0 w-1 h-full rounded-l-xl ${colorClasses[color as keyof typeof colorClasses].split(' ')[2]}`}
        ></div>
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              <div
                className={`flex items-center ${trend > 0 ? 'text-green-600 dark:text-green-400' : trend < 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-gray-400'}`}
              >
                {trend > 0 ? (
          ...
          <div
            className={`w-10 h-10 rounded-full ${colorClasses[color as keyof typeof colorClasses].split(' ')[2]} bg-opacity-10 dark:bg-opacity-20 flex items-center justify-center`}
          >
            <span
              className={`text-xs font-bold ${colorClasses[color as keyof typeof colorClasses].split(' ')[1]}`}
            >
              {score}
            </span>
          </div>
  ...
  const MetricCard = ({ title, value, change, trend }: MetricCardProps) => (
    <div className="bg-white dark:bg-gray-900/50 rounded-xl shadow-sm p-6 backdrop-blur-sm border border-gray-200 dark:border-gray-800/50 hover:shadow-md transition-all duration-200">
      <div className="flex items-center justify-between mb-3">
        ...
        <div
          className={`flex items-center text-sm px-2 py-1 rounded-full ${trend > 0 ? 'bg-green-100 dark:bg-green-900/20 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400'}`}
        >
          {trend > 0 ? (
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-blue-700 dark:text-blue-300 text-sm">
              Monitor your project performance, team efficiency, and delivery
              metrics to optimize workflow and achieve better results.
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Monthly project delivery and team collaboration metrics
              </p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-900/50 backdrop-blur-sm border border-gray-200 dark:border-gray-800/50 rounded-xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                Overall team productivity and task completion rate
              </p>
            </div>
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Track progress towards quarterly objectives and KPIs
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {targetData.map((target, index) => (
          <div
            key={index}
            className="bg-white dark:bg-gray-900/50 backdrop-blur-sm border border-gray-200 dark:border-gray-800/50 rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow"
          >
