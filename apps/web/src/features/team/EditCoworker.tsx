      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            email: member.email || '',
            isIndividual: member.is_individual || false,
            company: member.company || 'Choose company',
            source: member.source || 'Contact source',
            position: member.position || 'Contact person position',
            positionDescription: member.position_description || '',
            skype: member.skype || '',
            linkedin: member.linkedin || '',
            additionalLinks: member.additional_links || [],
            phoneNumbers: member.phone_numbers?.length ? member.phone_numbers : [''],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(errorData || 'Failed to update team member');
      }
    } catch (error) {
      ...
      <div className="bg-gray-900/50 backdrop-blur-sm border border-gray-800/50 rounded-xl">
        <div className="px-6 py-4 border-b border-gray-700/50">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-semibold text-white">Edit Coworker</h1>
