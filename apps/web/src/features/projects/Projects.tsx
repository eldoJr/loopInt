...
const Projects = ({
  ...
  const filteredProjects = (() => {
    ...
    const baseProjects = debouncedSearchTerm
    ...
    return baseProjects.filter(project => {
      const matchesDates =
        !filters.dates ||
        (project.deadline &&
          format(new Date(project.deadline), 'yyyy-MM-dd').includes(
            filters.dates
          )) ||
        (project.start_date &&
          format(new Date(project.start_date), 'yyyy-MM-dd').includes(
            filters.dates
          )) ||
        (project.deadline &&
          format(new Date(project.deadline), 'dd/MM/yyyy').includes(
            filters.dates
          )) ||
        (project.start_date &&
          format(new Date(project.start_date), 'dd/MM/yyyy').includes(
            filters.dates
          ));
      const matchesTags =
        !filters.tags ||
