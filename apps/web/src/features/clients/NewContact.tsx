...
import { X, Upload, Plus, AlertCircle, MapPin, Phone, Mail, Bold, Italic, Underline, Link, Strikethrough, List, ListOrdered, AlignLeft, AlignCenter, AlignRight, Code, Check } from 'lucide-react';
...
const NewClient = ({ onNavigateBack, onNavigateToNewContact }: NewClientProps) => {
  useTheme();
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!validTypes.includes(file.type)) {
        setErrors(prev => ({ ...prev, avatar: 'Please upload JPG, JPEG, PNG, or SVG format only' }));
        return;
      }
  ...
  const handleCancel = () => {
    if (formData.firstName.trim() || formData.lastName.trim() || formData.avatar || formData.email.trim()) {
      showConfirmation({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (formData.company.trim()) formDataToSend.append('company', formData.company.trim());
      if (formData.position.trim()) formDataToSend.append('position', formData.position.trim());
      ...
      <div className="bg-white dark:bg-gray-900/50 backdrop-blur-sm border border-gray-200 dark:border-gray-800/50 rounded-xl transition-all duration-300">
        <div className="px-4 py-3 border-b border-gray-200 dark:border-gray-700/50">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">New Client</h1>
