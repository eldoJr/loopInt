...
const Navbar = () => {
      ...
      items: [
        {
          ...
          description:
            'Streamline your projects with powerful collaboration tools',
        ...
        {
          label: 'Freshwear Management',
          href: '#freshwear-management',
          description: 'Complete inventory and retail management',
        ...
        },
        {
          label: 'Pricing',
          href: '#pricing',
          description: 'Flexible pricing plans for every business size',
      ...
      items: [
        {
          label: 'IT Solutions',
          href: '#marketing-it',
          description: 'Technology-focused marketing automation',
          icon: <BarChart2 className="w-4 h-4" />,
        },
        {
          label: 'HR Strategies',
          href: '#marketing-hr',
          description: 'Human resources marketing strategies',
          icon: <Users className="w-4 h-4" />,
        },
        {
          label: 'Sales Campaigns',
          href: '#marketing-sales',
          description: 'Data-driven sales campaign management',
          icon: <BarChart2 className="w-4 h-4" />,
          isNew: true,
        },
        {
          ...
          description: 'Creative team collaboration and workflows',
          icon: <LayoutDashboard className="w-4 h-4" />,
        },
        {
    ...
    useCases: {
      ...
      items: [
        ...
        },
        {
          label: 'HR Management',
          href: '#hr-usecase',
          description: 'Employee lifecycle and workflow management',
          icon: <Users className="w-4 h-4" />,
        },
        {
          label: 'Project Tracking',
          href: '#pm-usecase',
          description: 'Real-time team collaboration and tracking',
        ...
        {
          label: 'Freelance Tools',
          href: '#freelance-management',
          description: 'Everything independent professionals need',
          icon: <Zap className="w-4 h-4" />,
          isNew: true,
        },
      ],
  ...
  const renderDropdownItems = (
      ...
      return (
        <div className="space-y-1 py-2">
          {items.map((item, index) => (
            ...
            >
              <div
                className={`w-8 h-8 ${colorClasses.bg} rounded-lg mr-3 flex items-center justify-center flex-shrink-0 transition-all duration-200 group-hover:scale-105`}
              >
                {item.icon}
              </div>
              <div className="flex-1 min-w-0">
                ...
                {item.description && (
                  <div
                    className={`text-xs text-gray-500 mt-1 ${colorClasses.text} transition-colors duration-200 line-clamp-2`}
                  >
                    {item.description}
                  </div>
                )}
    ...
    return (
      <div
        className={`grid gap-3 ${itemChunks.length === 1 ? 'grid-cols-1' : itemChunks.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}
      >
            ...
            {chunk.map((item, index) => (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {item.description && (
                    <div
                      className={`text-xs text-gray-500 ${colorClasses.text} transition-colors duration-200 line-clamp-2 leading-relaxed`}
                    >
                      {item.description}
                    </div>
                  )}
    ...
    return (
      <div className="relative" key={sectionKey}>
        <button
          ...
          className={`flex items-center px-4 py-3 text-sm font-semibold transition-all duration-200 group relative rounded-lg ${
            activeDropdown === sectionKey
              ...
              : 'text-gray-300 hover:text-white hover:bg-gray-800/10'
          }`}
        ...
        >
          ...
          />
          <div
            className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 h-0.5 ${colorClasses.dot} transition-all duration-200 ${
              activeDropdown === sectionKey
                ? 'w-full'
        ...
        {activeDropdown === sectionKey && (
          <div
            ...
            style={{
              boxShadow:
                '0 20px 25px -5px rgba(0, 0, 0, 0.8), 0 0 0 1px rgba(255, 255, 255, 0.05)',
              maxWidth: 'calc(100vw - 2rem)',
            }}
          >
            <div className="relative z-10">
              <div
                className={`px-6 py-4 border-b border-gray-800/50 ${colorClasses.bg}`}
              >
                <h3 className="text-sm font-bold text-gray-200 flex items-center">
                  <div
                    className={`w-3 h-3 ${colorClasses.dot} rounded-full mr-3`}
                  ></div>
                  {section.title}
  ...
  const renderMobileDropdown = (sectionKey: string) => {
    ...
    return (
      <div
        key={sectionKey}
        className="border-b border-gray-800/20 last:border-b-0 rounded-lg overflow-hidden bg-gray-900/20"
      >
        <button
          onClick={e => {
            e.preventDefault();
        ...
        >
          <div className="flex items-center">
            <div
              className={`w-3 h-3 ${colorClasses.dot} rounded-full mr-3 transition-all duration-200 flex-shrink-0`}
            ></div>
  ...
  return (
    <>
      <nav
        className={`fixed w-full z-50 bg-gray-900/95 shadow-sm border-b border-white/5 backdrop-blur-sm transition-all duration-300 ${
      ...
      >
        <div className="max-w-[90rem] mx-auto px-6 sm:px-8 lg:px-12">
          <div className="flex items-center justify-between h-16">
            ...
            {/* Desktop Menu */}
            <div
              className="hidden lg:flex items-center space-x-6 ml-auto mr-6"
              ref={dropdownRef}
            >
              <a
                href="#about"
              ...
              {Object.keys(navigationData).map(sectionKey =>
                renderDesktopDropdown(sectionKey)
              )}
              <a
                href="#contact"
                className="px-4 py-3 text-sm font-semibold text-gray-300 hover:text-white transition-all duration-200 hover:bg-gray-800/10 rounded-lg relative group tracking-wide"
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4 space-y-3">
              ...
              <div className="space-y-2">
                {Object.keys(navigationData).map(sectionKey =>
                  renderMobileDropdown(sectionKey)
                )}
              </div>
              <div className="space-y-1">
                <a
