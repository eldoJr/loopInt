...
const DashboardListItem = ({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'completed': case 'done': return 'bg-green-500/20 text-green-400 border-green-500/30';
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-3">
        ...
        <div>
          <span className={`text-gray-300 font-medium ${completed ? 'line-through opacity-60' : ''}`}>
            {title}
          </span>
          {subtitle && (
