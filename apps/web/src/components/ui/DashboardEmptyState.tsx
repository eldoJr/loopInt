...
const DashboardEmptyState = ({
  ...
  const finalActions = actions || (actionText && onAction ? [{ text: actionText, onClick: onAction, variant: 'primary' as const }] : []);
  
  // Default icon based on message content
  const DefaultIcon = Icon || (
    message.toLowerCase().includes('task') ? FileX :
    message.toLowerCase().includes('project') ? FolderOpen :
    message.toLowerCase().includes('team') ? Users : Plus
  );
