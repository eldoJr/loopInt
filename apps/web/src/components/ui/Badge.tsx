...
const Badge = ({ children, icon: Icon, variant = 'primary' }: BadgeProps) => {
  const variants = {
    ...
    success:
      'bg-gradient-to-r from-emerald-500/10 to-green-500/10 border-emerald-500/20 text-emerald-400',
    warning:
  ...
  return (
    <div
      className={`inline-flex items-center px-4 py-2 border rounded-full text-sm font-medium backdrop-blur-sm ${variants[variant]}`}
    >
